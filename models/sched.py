...
def get_lr_sched(global_step, opts, args):
    # learning rate scheduling
    lr_this_step = opts['init_lr'] * warmup_linear(
        global_step, opts['warmup_steps'], int(args.total_samples//(opts['batch_size_train']*opts['n_gpu'])*opts['max_epoch']))
    if lr_this_step <= 0:
        lr_this_step = 1e-8
    return lr_this_step
