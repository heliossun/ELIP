...
We did not compare `ELIP` with the Multi-Modal LLM `GPT4V` in our project since ELIP is an encoder-based model, which is different from GPT4V, a large generative model. Also, the limited computation power makes it hard to follow a training scale similar to GPT4V's.
...
* BLIP has three loss terms: ITC, ITM, and LM. For ITC loss, they introduced a `momentum encoder` to generate soft labels, and this module aims to support the model performing image-text retrieval and image captioning. However, our project mainly focuses on image-text retrieval, so we follow BLIP’s implementation but simplify ELIP+ architecture by eliminating all the momentum encoders.
...
TODO: [ensemble adapters experiments, take average performance. 3 / 5
Or Ensemble last few layers of CLIP. 3 / 5
Variation drop-out in adapter (optional).
Adversarial attack loss in training?]
...
`A5`:  Our method is an efficient fine-tuning method, that all pre-trained CLIP weights are fixed during training. With this lightweight fine-tuning method and evidential loss, our method can scale to most of the transformer-based methods that are trained using contrastive loss.
